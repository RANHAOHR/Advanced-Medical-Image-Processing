function snake_shape(binim, forcetype, std, support, Niter, Nsample, alpha, beta, gamma, extcoef, balcoef, itergvf)
...
if forcetype == 2
    ...
    quiver(dX,dY);
    figure(5);
    mesh(1:256,1:256,dist_transform);
end
% ---------------- METHOD: GVF -------------------
if forcetype == 3
    ...
    for i = 1:itergvf
        ...
        u_matrix = u_matrix .* (ones(256) - b_xy *delta_t)+ r*(u_i_right+ u_j_down+u_i_left + u_j_up -4*u_matrix) + cx_ * delta_t;
        v_matrix = v_matrix .* (ones(256) - b_xy *delta_t)+ r*(v_i_right+ v_j_down+v_i_left + v_j_up -4*v_matrix) + cy_ * delta_t;
